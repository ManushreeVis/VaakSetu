# VaakSetu — Desktop (`.exe`) Build Guide

VaakSetu ships as a **standalone Windows desktop application** (also macOS/Linux) via
...
- `VaakSetu-1.0.0-x64.exe` — **NSIS installer** (per-machine, desktop + Start-menu shortcuts)
- `VaakSetu-1.0.0-x64.exe` (portable) — single-file portable build
...
|---|---|---|
| VaakSetu UI + server | `.next/standalone` | MIT (project) |
...
| SQLite DB | `userData/vaaksetu.db` | public domain |
...
1. Double-click `VaakSetu-Setup-1.0.0.exe`.
2. Choose install directory (default `C:\Program Files\VaakSetu`).
...
4. Translations, transcriptions and generated files are stored under
   `%APPDATA%\VaakSetu\storage\` and `%APPDATA%\VaakSetu\vaaksetu.db`.

## 9. Updating models without rebuilding

Drop new/updated weights into `%PROGRAMDATA%\VaakSetu\models\` (or the `extraResources`
